...
function isState(value: unknown): value is ExternalMissionState {
  ...
  return record.revision === REVISION && typeof config === 'object' && config !== null &&
    typeof config.id === 'string' && typeof config.generation === 'string' && Array.isArray(config.probeArgv) &&
    ...
    Number.isSafeInteger(config.pollIntervalMs) && Number.isSafeInteger(config.probeTimeoutMs) &&
    ...
    Number.isSafeInteger(record.probeCount) && typeof record.updatedAt === 'string';
}
...
function makeReceipt(config: ExternalMissionConfig, observation: WaitObservation, reason: WaitWakeReason, evidence: ExternalMissionReceipt['evidence']): ExternalMissionReceipt {
  return {
    id: `mission-receipt:${config.id}:${config.generation}`,
    missionId: config.id,
    generation: config.generation,
    reason,
    status: observation.status as ExternalMissionReceipt['status'],
...
function callWake(config: ExternalMissionConfig, store: ExternalMissionStore, receipt: ExternalMissionReceipt): void {
  ...
  execFileSync(file, args, {
    ...
    env: { ...process.env, TACHIKO_MISSION_RECEIPT_ID: receipt.id, TACHIKO_MISSION_RECEIPT_PATH: store.filePath, TACHIKO_MISSION_ID: config.id },
  });
}
...
export async function superviseExternalMissionAsync(
  ...
  try {
    ...
    for (;;) {
      ...
      state = { ...state, previous: observation, probeCount: state.probeCount + 1, updatedAt: new Date().toISOString() };
      store.write(state);
      polls += 1;
