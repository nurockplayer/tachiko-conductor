...
Issue Forms are structured intake. For repository collaborators (`OWNER`, `MEMBER`, or `COLLABORATOR`), a complete form may become trusted task authority. For untrusted authors, the same form is only a proposal and receives `needs:steward`; it cannot receive `dispatch:ready`.
