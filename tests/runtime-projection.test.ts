...
it('hold survives reload and release is idempotent', () => { const value = dir(); setMaintenanceHold(value, true, 't'); assert.equal(readOperationalRuntimeProjection(value)?.maintenanceHold.active, true); setMaintenanceHold(value, false, 't'); setMaintenanceHold(value, false, 't'); assert.equal(readOperationalRuntimeProjection(value)?.maintenanceHold.active, false); });
