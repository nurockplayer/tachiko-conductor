...
DEFAULT_PROMPT = (
    ...
    "standing SCD policy. Reconcile already-active owned work first and use live GitHub "
    "authority. For a "
    "long-running worker, CI job, or subprocess, use `tachiko mission wait start` "
    "with a bounded source-of-truth probe, immutable owner/session/worktree/cwd identity, "
    "and a durable continuation callback when that callback is actually configured; "
    "then end this turn without polling status or waiting on the monitor. This heartbeat "
    "has no native ChatGPT Desktop continuation API or stable session binding, so do not "
    "start a callback wait unless an explicit Codex session id and callback are available. "
    "Stay quiet when no work or meaningful update is executable. Only "
